...
from routing_module.routing import find_journeys, load_traffic, explore_trips, TripPricePredictor
...
class RoutingServiceServicer(routing_pb2_grpc.RoutingServiceServicer):
    ...
    def FindRoute(self, request, context):
        ...
        try:
            ...
            if graph is None or trip_graph is None or pathways_dict is None or traffic is None:
                context.set_code(grpc.StatusCode.UNAVAILABLE)
                context.set_details("Server is still loading network data. Please try again in a moment.")
            ...
            start_trips = explore_trips(graph, start_node, cutoff=request.walking_cutoff)
            ...
            if not start_trips:
                ...
                context.set_details(f"No transit trips found within {request.walking_cutoff}m of start location")
                return routing_pb2.RouteResponse()
            
            if not target_trips:
                context.set_code(grpc.StatusCode.NOT_FOUND)
                context.set_details(f"No transit trips found within {request.walking_cutoff}m of end location")
                return routing_pb2.RouteResponse()

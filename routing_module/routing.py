...
class RoutingEngine:
    ...
    def __init__(self, params_path=None):
        ...
        # Graph and lookup data - to be loaded via setter methods
        self.graph = None  # OSM street network graph
        self.trip_graph = None  # Transit trip graph
        self.pathway_metadata = None  # Pathway metadata for enrichment
        self.enrichment_lookups = None  # Lookup tables for enrichment
    ...
    def set_graph_data(self, graph, trip_graph, pathway_metadata, enrichment_lookups):
        """Set the graph and lookup data for routing.
        Args:
        ...
        """
        self.graph = graph
        self.trip_graph = trip_graph
        self.pathway_metadata = pathway_metadata
        self.enrichment_lookups = enrichment_lookups

    def _ensure_graph_data_loaded(self):
        """Ensure graph data is loaded before routing operations.

        Raises:
            RuntimeError: If graph data hasn't been set
        """
        if self.graph is None or self.trip_graph is None:
            raise RuntimeError("Graph data not loaded. Call set_graph_data() first.")
    ...
    def find_journeys(self, start_trips, goal_trips, max_transfers):
        """Find optimal journeys using Pareto optimization."""
        self._ensure_graph_data_loaded()
        ...
        while queue:
            ...
            for next_trip, lean_pathway in self.trip_graph.get(
                current_trip, {}
            ).items():
    ...
    def explore_trips(self, source, cutoff=float("inf")):
        """
        ...
        while pq:
            ...
            if not access_map:
                access_map = self.graph.nodes[node].get("boarding_map")
                if access_map:
    ...
    def enrich_journey_results(self, lean_journeys, start_trips, target_trips):
        """
        ...
        trip_to_route = self.enrichment_lookups["trip_to_route"]
        route_to_name = self.enrichment_lookups["route_to_name"]
        route_to_short_name = self.enrichment_lookups["route_to_short_name"]
        trip_to_headsign = self.enrichment_lookups["trip_to_headsign"]
        stop_to_coords = self.enrichment_lookups["stop_to_coords"]
        stop_to_name = self.enrichment_lookups["stop_to_name"]
            ...
            for i, current_trip_id in enumerate(trip_path):
                ...
                if i == 0:
                    ...
                    if i == len(trip_path) - 1:
                        ...
                        lean_pathway = self.trip_graph.get(current_trip_id, {}).get(
                            next_trip_id, {}
                        )
                        to_stop_id = lean_pathway.get("start_stop_id")
                        to_coords = (
                            stop_to_coords.get(to_stop_id, {}) if to_stop_id else {}
                        )
                else:
                    # Subsequent trips: from transfer point
                    prev_trip_id = trip_path[i - 1]
                    lean_pathway = self.trip_graph.get(prev_trip_id, {}).get(
                        current_trip_id, {}
                    )
                ...
                trip_duration = max(
                    5, int(total_time / len(trip_path))
                )  # Rough estimate
                trip_fare = round(
                    total_fare / len(trip_path), 2
                )  # Split fare across trips
                ...
                trip_leg = {
                    ...
                    "mode": mode,
                    "route_name": route_to_name.get(route_id, "Unknown Route"),
                ...
                if i < len(trip_path) - 1:
                    ...
                    pathway_key = (current_trip_id, next_trip_id)
                    walking_coords = []
                    if pathway_key in self.pathway_metadata:
                        pathway_info = self.pathway_metadata[pathway_key]
        ...
        # Sort journeys by: 1) least transfers, 2) least time, 3) least cost
        detailed_journeys.sort(
            key=lambda j: (
                j["summary"]["transfers"],
                j["summary"]["total_time_minutes"],
                j["summary"]["cost"],
            )
        )
    ...
    def find_route(
        ...
        lean_journeys = self.find_journeys(
        ...
        enriched_results = self.enrich_journey_results(
...
def create_routing_engine(
    params_path=None, osm_file=None, gtfs_path=None, pathways_file=None
):
    """
    Convenience function to create a fully initialized RoutingEngine.

    Args:
        params_path: Path to the price prediction model parameters
    ...
    Returns:
        Fully initialized RoutingEngine instance ready for routing
    """
    print("Creating and initializing RoutingEngine...")

    # Create network data
    graph, trip_graph, pathway_metadata, enrichment_lookups = create_network(
        osm_file=osm_file, gtfs_path=gtfs_path, pathways_file=pathways_file
    )

    # Initialize routing engine
    engine = RoutingEngine(params_path=params_path)

    # Load graph data
    engine.set_graph_data(graph, trip_graph, pathway_metadata, enrichment_lookups)

    print("RoutingEngine initialization complete!")
    return engine

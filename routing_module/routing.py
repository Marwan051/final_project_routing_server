...
def get_distance(trip_id, start_stop, end_stop, distances_path=None):
    ...
    if distances_path is None:
        distances_path = os.path.join(os.path.dirname(__file__), 'data', 'utils', 'trip_distances.csv')
...
def load_traffic(traffic_path=None):
    ...
    if traffic_path is None:
        traffic_path = os.path.join(os.path.dirname(__file__), 'data', 'prefixtimes.json')
...
def find_journeys(graph, pathways_dict, start_trips, goal_trips, max_transfers, traffic=None):
    """
        ...
        for next_trip, pathway_id in graph.get(current_trip, {}).items():
            ...
            if is_potentially_useful:
                ...
                if next_trip in goal_trips:
                    ...
                    last_leg_time = get_transport_time(next_trip, pathway['end_stop_id'], goal_stop, traffic)

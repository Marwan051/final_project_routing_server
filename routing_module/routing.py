...
class RoutingEngine:
    ...
    def _ensure_graph_data_loaded(self):
        ...
        if self.graph is None or self.trip_graph is None:
            raise RuntimeError(
                "Graph data not loaded. Call set_graph_data() first."
            )
    ...
    def find_journeys(self, start_trips, goal_trips, max_transfers):
        ...
        while queue:
            ...
            for next_trip, lean_pathway in self.trip_graph.get(current_trip, {}).items():
                ...
                if next_trip in goal_trips:
                    ...
                    if transfer_in_seq < goal_seq:
                        ...
                        last_leg_time = self.get_transport_time(next_trip, lean_pathway['end_stop_id'], goal_stop)
                        last_leg_money = self.get_fare(next_trip, lean_pathway['end_stop_id'], goal_stop, goal_trips[next_trip]['agency'])
            ...
            for i, current_trip_id in enumerate(trip_path):
                ...
                if i == 0:
                    ...
                    if i == len(trip_path) - 1:
                        ...
                        lean_pathway = self.trip_graph.get(current_trip_id, {}).get(next_trip_id, {})
                    ...
                    lean_pathway = self.trip_graph.get(prev_trip_id, {}).get(current_trip_id, {})
...
def create_routing_engine(params_path=None, osm_file=None, gtfs_path=None, pathways_file=None):
    """
